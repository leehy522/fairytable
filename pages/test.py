...
def show_margin_calc():
    st.title("🛡️ 페어리테이블 마진 정밀 시뮬레이터 (V2.0)")
    st.markdown("---")
    ...
    try:
        ...
        # [핵심수정] 원료 단가 추출 시 '신재', '재생' 키워드가 포함된 컬럼을 유연하게 찾습니다.
        sinjae = clean_num(next((target_cost_row[k] for k in target_cost_row.index if '신재' in k), 0))
        jaesaeng = clean_num(target_cost_row[k] for k in target_cost_row.index if '재생' in k) if any('재생' in k for k in target_cost_row.index) else 0
        jaesaeng = clean_num(jaesaeng)
        ...
        st.dataframe(df_res.style.applymap(lambda v: 'color: red;' if '🚨' in str(v) else '', subset=[COL_STATUS]), use_container_width=True, hide_index=True)
