...
def show_margin_calc():
    st.title("🛡️ 페어리테이블 마진 정밀 시뮬레이터 (V3.2)")
    st.markdown("---")

    try:
        # 1. 데이터 로드
        sheet_id = "13ldXPSVT7CFyNZRj-6Rlv3aXMqOhflquUtcZom5cJzU"
        sheet_name_1, sheet_name_2, sheet_name_3 = quote("상품목록"), quote("원가기준"), quote("월별납품가")
        
        df_p = pd.read_csv(f"https://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?tqx=out:csv&sheet={sheet_name_1}")
        df_c = pd.read_csv(f"https://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?tqx=out:csv&sheet={sheet_name_2}")
        df_m = pd.read_csv(f"https://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?tqx=out:csv&sheet={sheet_name_3}")
        ...
        # [핵심수정] 원료 단가 추출 로직 강화 (비율 컬럼과 단가 컬럼 엄격 구분)
        ...
        e_output = st.data_editor(edit_df, use_container_width=True, key="v32_sync")
        ...
        st.download_button("📥 엑셀 다운로드", buffer.getvalue(), f"마진분석_{sel_month}.xlsx")
