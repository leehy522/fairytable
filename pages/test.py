...
def show_margin_calc():
    st.title("🛡️ 페어리테이블 마진 정밀 시뮬레이터 (V3.5)")
    st.markdown("---")
    ...
    try:
        ...
        # ---------------------------------------------------------
        # [수정] 3. 지능형 단가 열 탐색 (상품명 제외)
        # ---------------------------------------------------------
        # 단가 열이 아닐 가능성이 높은 키워드 목록
        exclude_keywords = ['상품명', 'SKU', '코드', '비고', '규격', '단위', '매수']
        
        # 제외 키워드가 포함되지 않은 열들만 후보로 선정
        price_candidates = [c for c in df_m.columns if not any(k in c.upper() for k in exclude_keywords)]
        
        if sel_month in df_m.columns:
            p_col = sel_month
        else:
            # 후보 열 중에서 '기본' 혹은 'DEFAULT' 찾기
            p_col = next((c for c in price_candidates if '기본' in c or 'DEFAULT' in c.upper()), None)
            # 없으면 후보 열 중 가장 첫 번째 열 사용
            if not p_col and price_candidates:
                p_col = price_candidates[0]
            elif not p_col:
                p_col = df_m.columns[1] # 최후의 수단
        ...
        e_output = st.data_editor(edit_df, use_container_width=True, key="v35_fixed")
        ...
        st.download_button("📥 엑셀 리포트 다운로드", buffer.getvalue(), f"마진분석_{sel_month}.xlsx")
