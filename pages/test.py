...
def show_margin_calc():
    ...
    try:
        ...
        # 4. 분석 로직 (기존 계산식 유지 + 상품당 수익 추가)
        COL_SKU, COL_NAME, COL_COST, COL_APPLIED, COL_REC, COL_ADJ, COL_COUPANG, COL_UNIT_PROFIT, COL_PROFIT, COL_STATUS = \
            'SKU ID', '상품명', '제조원가(박스)', '적용납품가', '추천납품가', '단가 조정액(+/-)', '쿠팡판매가(42%)', '상품당수익', '롤당수익', '방어선'
        ...
        def calc_logic(row):
            try:
                ...
                # 수익 계산 (상품당 수익 추가)
                unit_profit = applied_p - total_box_cost  # 박스 1개당 수익
                ...
                current_roll_profit = unit_profit * boxes_per_roll
                ...
                return pd.Series([
                    ...
                    fmt(rec_nap_ga), fmt_adj(adj_val), fmt(coupang_selling), fmt(unit_profit), fmt(current_roll_profit), status
                ], index=[COL_SKU, COL_NAME, COL_COST, COL_APPLIED, COL_REC, COL_ADJ, COL_COUPANG, COL_UNIT_PROFIT, COL_PROFIT, COL_STATUS])
            except:
                return pd.Series([sku_id, row.get('상품명', ''), '0원', '0원', '0원', '0원', '0원', '0원', '0원', '오류'], 
                                 index=[COL_SKU, COL_NAME, COL_COST, COL_APPLIED, COL_REC, COL_ADJ, COL_COUPANG, COL_UNIT_PROFIT, COL_PROFIT, COL_STATUS])
