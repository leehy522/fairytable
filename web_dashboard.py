        ...
        if shape.has_table:
            ...
            try:
                ...
                for idx, item in enumerate(p['items_list']):
                    ...
                    real_qty = str(item.get('qty', 0))
                    
                    # 3. 발주수량 및 입고확인 칸에 해당 상품의 개별 수량 입력
                    set_bold_text(table.cell(row_idx, 3).text_frame, real_qty, False) # 발주수량
                    set_bold_text(table.cell(row_idx, 4).text_frame, real_qty, False) # 입고확인
