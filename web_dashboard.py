...
# --- [1. 데이터 연동 설정] ---
# 💡 윤겸님의 엑셀 직통 다운로드 주소 (resid 방식)
# 예: "https://onedrive.live.com/download?resid=40F78A9D17F33324!105"
EXCEL_URL = "https://onedrive.live.com/download?resid=40F78A9D17F33324!s8899948b6b8c45babf6b75bda192b190"

@st.cache_data(ttl=60) # 1분마다 엑셀의 변경사항을 확인합니다.
def load_excel_data():
    try:
        response = requests.get(EXCEL_URL)
        f = io.BytesIO(response.content)
        # 엑셀의 6행부터 데이터가 시작된다고 가정 (skiprows=5)
        df = pd.read_excel(f, skiprows=5, engine='openpyxl')
        
        # 컬럼명 앞뒤 공백 제거 및 소문자 변환 (유지보수 용이)
        df.columns = [str(c).strip().lower() for c in df.columns]
        return df.dropna(subset=['name']) # 이름 없는 빈 행 제거
    except Exception as e:
...
if not df.empty:
    # --- [3. 윤겸님의 황금 공식 적용] ---
    # 공식: (폭/100) * (길이/100) * 2 * 92 * 두께 * 매수
    try:
        # 데이터 숫자형 변환
        for col in ['width', 'length', 'thickness', 'count', 'price']:
            df[col] = pd.to_numeric(df[col], errors='coerce')

        # 무게 및 원가 계산
        df['weight_kg'] = (df['width']/100) * (df['length']/100) * 2 * 92 * df['thickness'] * df['count']
        df['cost'] = df['weight_kg'] * m_price
        df['margin_rate'] = (1 - (df['cost'] / df['price'])) * 100

        # --- [4. 화면 출력] ---
        st.subheader("📦 상품별 실시간 원가/마진 분석")
        st.info("내 컴퓨터에서 엑셀 저장 시 1분 내 자동 반영됩니다.")
        
        # 보기 좋게 포맷팅하여 출력
        display_df = df[['no', 'name', 'width', 'length', 'thickness', 'count', 'price', 'weight_kg', 'cost', 'margin_rate']]
        st.dataframe(display_df.style.format({
            'price': '₩{:,.0f}',
            'weight_kg': '{:.3f}kg',
            'cost': '₩{:,.0f}',
            'margin_rate': '{:.1f}%'
        }).background_gradient(subset=['margin_rate'], cmap='RdYlGn'))

    except Exception as e:
        st.warning(f"계산 중 오류 발생: {e}. 엑셀의 숫자 칸을 확인해 주세요.")
